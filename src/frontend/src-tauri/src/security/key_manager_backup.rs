...
impl<C: KeyConfig> KeyManager<C> {
    ...
    fn save_key_store(&self, key_store: &EncryptedKeyStore) -> Result<(), KeyError> {
        println!("[DEBUG] save_key_store: Start - Pfad: {}", self.store_path);
        
        // Serialisiere den Schlüsselspeicher
        println!("[DEBUG] save_key_store: Serialisiere Daten...");
        let store_data = serde_json::to_vec(key_store)
            .map_err(|e| KeyError::Serialization(e.to_string()))?;
        println!("[DEBUG] save_key_store: Serialisierung erfolgreich ({} Bytes)", store_data.len());
            
        // Erstelle Verzeichnis, falls es nicht existiert
        println!("[DEBUG] save_key_store: Prüfe Verzeichnis...");
        if let Some(parent) = Path::new(&self.store_path).parent() {
            println!("[DEBUG] save_key_store: Erstelle Verzeichnis: {:?}", parent);
            fs::create_dir_all(parent)
                .map_err(|e| KeyError::Io(e))?;
            println!("[DEBUG] save_key_store: Verzeichnis erstellt");
        }
        
        // Schreibe den Schlüsselspeicher
        println!("[DEBUG] save_key_store: Schreibe Datei...");
        fs::write(&self.store_path, store_data)
            .map_err(|e| KeyError::Io(e))?;
        println!("[DEBUG] save_key_store: Datei geschrieben");
            
        println!("[DEBUG] save_key_store: Erfolgreich abgeschlossen");
        Ok(())
    }
    ...
    pub fn rotate_key(&self, key_type: KeyType, user_id: &str) -> Result<(), KeyError> {
        println!("[DEBUG] rotate_key: Start für {:?}", key_type);
        
        // Prüfe, ob der KeyManager initialisiert ist
        println!("[DEBUG] rotate_key: Prüfe Initialisierung...");
        let is_initialized = {
            let store = self.key_store.lock().unwrap();
            let result = store.is_some();
            println!("[DEBUG] rotate_key: Initialisierung = {}", result);
            result
        };

        if !is_initialized {
            println!("[DEBUG] rotate_key: Nicht initialisiert - Fehler");
            return Err(KeyError::StoreNotInitialized);
        }

        // Audit-Log für Rotation-Start [ATV]
        println!("[DEBUG] rotate_key: Schreibe Audit-Log...");
        self.audit_logger.log(
            "key_manager", 
            user_id,
        ...
        println!("[DEBUG] rotate_key: Audit-Log geschrieben");

        // Generiere neuen Schlüssel
        println!("[DEBUG] rotate_key: Generiere neuen Schlüssel...");
        let new_key = self.generate_random_key()?;
        println!("[DEBUG] rotate_key: Neuer Schlüssel generiert ({}B)", new_key.len());

        // Generiere neuen IV
        println!("[DEBUG] rotate_key: Generiere neuen IV...");
        let mut new_iv = [0u8; 12];
        OsRng.fill_bytes(&mut new_iv);
        println!("[DEBUG] rotate_key: Neuer IV generiert");
        
        // Hole Master-Schlüssel und aktualisiere aktiven Schlüssel in einem Zug [FSD]
        println!("[DEBUG] rotate_key: Hole Master-Schlüssel...");
        let master_key = {
            println!("[DEBUG] rotate_key: Versuche active_keys Lock...");
            let mut active_keys = self.active_keys.lock().unwrap();
            println!("[DEBUG] rotate_key: active_keys Lock erhalten");
            
            let master_key = active_keys.get(&KeyType::Master)
                .ok_or(KeyError::KeyNotFound(KeyType::Master))?
                .clone();
            println!("[DEBUG] rotate_key: Master-Schlüssel geholt");
            
            // Aktualisiere den aktiven Schlüssel sofort im gleichen Lock-Bereich
            active_keys.insert(key_type, new_key.clone());
            println!("[DEBUG] rotate_key: Aktiver Schlüssel aktualisiert");
            
            master_key
        }; // Lock wird hier automatisch freigegeben
        println!("[DEBUG] rotate_key: active_keys Lock freigegeben");
        
        // Verschlüssele neuen Schlüssel
        println!("[DEBUG] rotate_key: Verschlüssele neuen Schlüssel...");
        let encrypted_new_key = self.encrypt_with_master(&master_key, &new_key, &new_iv)?;
        println!("[DEBUG] rotate_key: Schlüssel verschlüsselt");
        
        // Aktualisiere den Schlüsselspeicher [ZTS]
        println!("[DEBUG] rotate_key: Aktualisiere Schlüsselspeicher...");
        {
            println!("[DEBUG] rotate_key: Versuche key_store Lock...");
            let mut store = self.key_store.lock().unwrap();
            println!("[DEBUG] rotate_key: key_store Lock erhalten");
            ...
            println!("[DEBUG] rotate_key: Verschlüsselter Schlüssel gespeichert");
            
            // Aktualisiere IV
            key_store.ivs.insert(key_type, new_iv);
            println!("[DEBUG] rotate_key: IV gespeichert");
            ...
            if let Some(metadata) = key_store.key_metadata.get_mut(&key_type) {
                ...
                metadata.version += 1;
                println!("[DEBUG] rotate_key: Metadaten aktualisiert");
            }
            
            // Speichere aktualisierten Schlüsselspeicher
            println!("[DEBUG] rotate_key: Speichere Schlüsselspeicher...");
            self.save_key_store(key_store)?;
            println!("[DEBUG] rotate_key: Schlüsselspeicher gespeichert");
        } // Lock wird hier automatisch freigegeben
        println!("[DEBUG] rotate_key: key_store Lock freigegeben");
    ...
    // Leitet Master-Schlüssel vom Passwort ab [SP]
    fn derive_master_key(&self, password: &str, salt: &[u8]) -> Result<Vec<u8>, KeyError> {
        // Argon2id-Parameter für sichere Schlüsselableitung
...
pub fn get_key(&self, key_type: KeyType) -> Result<Vec<u8>, KeyError> {
    ...
    // Sammle alle benötigten Daten aus dem Schlüsselspeicher [FSD]
    let (encrypted_key, iv, master_key) = {
        let store = self.key_store.lock().unwrap();
        ...
        // Hole Master-Schlüssel aus active_keys in separatem Block
        let master_key = {
            let active_keys = self.active_keys.lock().unwrap();
        ...
        (encrypted_key, iv, master_key)
    }; // key_store Lock wird hier freigegeben
        
    // Entschlüssele den Schlüssel
    let key = self.decrypt_with_master(&master_key, &encrypted_key, &iv)?;
    
    // Speichere den Schlüssel im Speicher [ZTS]
    {
        let mut active_keys = self.active_keys.lock().unwrap();
